...
--(4) Count the number of unique product categories from the Products table.
SELECT COUNT(DISTINCT Category)
FROM Products;

--(5) Find the total sales amount for the product with id 7 in the Sales table.
SELECT SUM(SaleAmount)
FROM Sales
WHERE ProductID = 7;

--(6) Calculate the average age of employees in the Employees table.
...
--(8) Show the minimum and maximum price of products grouped by Category using the Products table.
SELECT Category, MIN(Price) AS MinPrice, MAX(Price) AS MaxPrice
FROM Products
GROUP BY Category;

--(9) Calculate the total sales per customer in the Sales table.
SELECT CustomerID, SUM(SaleAmount) AS [Total sales per Customer]
FROM Sales
GROUP BY CustomerID;

--(10) Filter departments having more than 5 employees from the Employees table.
...
-- (11) Calculate the total sales and average sales for each product category from the Products table.
SELECT Category, SUM(Price * StockQuantity) AS [Total sales], AVG(Price * StockQuantity) AS [Average sales]
FROM Products
GROUP BY Category;

-- (12) Count the number of employees from the Department HR.
SELECT DepartmentName, COUNT(EmployeeID)
FROM Employees
WHERE DepartmentName = 'HR'
GROUP BY DepartmentName;

-- (13) Find the highest and lowest salary by department in the Employees table.
SELECT DepartmentName, MAX(Salary) AS [Highest salary], MIN(Salary) AS [Lowest salary]
FROM Employees
GROUP BY DepartmentName;

-- (14) Calculate the average salary per department.
SELECT DepartmentName, CAST(AVG(Salary) AS FLOAT) AS [Average salary]
FROM Employees
GROUP BY DepartmentName;

-- (15) Show the average salary and count of employees working in each department.
SELECT DepartmentName, CAST(AVG(Salary) AS FLOAT) AS [Average salary], COUNT(EmployeeID) AS [Number of employees]
FROM Employees
GROUP BY DepartmentName;

-- (16) Filter product categories with an average price greater than 400.
SELECT Category, AVG(Price)
FROM Products
GROUP BY Category
HAVING AVG(Price) > 400;

-- (17) Calculate the total sales for each year in the Sales table.
SELECT YEAR(SaleDate) AS [Year], SUM(SaleAmount) AS [Total sales]
FROM Sales
GROUP BY YEAR(SaleDate);

-- (18) Show the list of customers who placed at least 3 orders.
SELECT CustomerID, COUNT(*) AS OrderCount
FROM Orders
GROUP BY CustomerID
HAVING COUNT(*) >= 3;

-- (19) Filter out departments with average salary expenses greater than 60000.
SELECT DepartmentName, CAST(AVG(Salary) AS FLOAT) AS AverageSalary
FROM Employees
GROUP BY DepartmentName
...
-- (20) Show the average price for each product category, and then filter categories with an average price greater than 150.
SELECT Category, AVG(Price) AS [Average Price]
FROM Products
GROUP BY Category
HAVING AVG(Price) > 150;

-- (21) Calculate the total sales for each customer, then filter the results to include only customers with total sales over 1500.
SELECT CustomerID, SUM(SaleAmount) AS [Total Sales]
FROM Sales
GROUP BY CustomerID
HAVING SUM(SaleAmount) > 1500;

-- (22) Find the total and average salary of employees in each department, and filter the output to include only departments with an average salary greater than 65000.
SELECT DepartmentName, SUM(Salary) AS [Total Salary], AVG(Salary) AS [Average Salary]
FROM Employees
GROUP BY DepartmentName
HAVING AVG(Salary) > 65000;

-- (23) Find the total amount for orders weighing more than $50 for each customer along with their least purchases (least amount might be lower than 50, use TSQL2012.Sales.Orders table, freight column).
SELECT CustomerID, SUM(TotalAmount) AS TotalAmount, MIN(TotalAmount) AS LeastPurchase
FROM Orders
GROUP BY CustomerID
HAVING SUM(TotalAmount) > 50;

-- (24) Calculate the total sales and count unique products sold in each month of each year, and then filter the months with at least 2 products sold (Orders).
SELECT  
	YEAR(OrderDate) AS [Year], 
	MONTH(OrderDate) AS [Month], 
	SUM(TotalAmount) AS [Total Sales],
	COUNT(DISTINCT ProductID) AS [Unique Product]
FROM Orders
GROUP BY YEAR(OrderDate), MONTH(OrderDate)
HAVING COUNT(DISTINCT ProductID) >= 2
ORDER BY [Year], [Month]

-- (25) Find the MIN and MAX order quantity per year from the Orders table.
SELECT YEAR(OrderDate) AS SaleYear, MIN(Quantity) AS MinOrder, MAX(Quantity) AS MaxOrder
FROM Orders
