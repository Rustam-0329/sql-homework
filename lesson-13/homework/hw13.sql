...
SELECT CONCAT(EMPLOYEE_ID, '-', FIRST_NAME, ' ', LAST_NAME) AS Emp_IDwithFullName
FROM Employees
...
UPDATE Employees
SET PHONE_NUMBER = STUFF(PHONE_NUMBER, 5, 3, 999)
...
SELECT *
FROM SingleOrder
ORDER BY CASE Id WHEN 0 THEN Vals
	     END ASC;
...
SELECT
    ...
    TRIM(CASE WHEN CHARINDEX(' ', FullName, CHARINDEX(' ', FullName + ' ') + 1) > 0 
    ...
    TRIM(CASE WHEN CHARINDEX(' ', FullName, CHARINDEX(' ', FullName + ' ') + 1) > 0 
        ...
        ELSE SUBSTRING(FullName, CHARINDEX(' ', FullName + ' ') + 1, LEN(FullName)) END) AS Lastname
FROM Students;
...
SELECT STUFF(
				(SELECT ' ' + String 
				 FROM DMLTable ORDER BY SequenceNumber 
				 FOR XML PATH('')), 1, 1, ''
...
FROM Employees
WHERE (LEN(CONCAT(FIRST_NAME, ' ', LAST_NAME)) - LEN(REPLACE(LOWER(CONCAT(FIRST_NAME, ' ', LAST_NAME)), 'a', ''))) >= 3;
...
SELECT 
    ...
    SUM(CASE WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) >= 3 THEN 1 ELSE 0 END) AS EMP_OVER_3_YEARS,
    (100 * SUM(CASE WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) >= 3 THEN 1 ELSE 0 END) / COUNT(EMPLOYEE_ID)) AS PERCENT_OVER_3_YEARS
FROM Employees
...
SELECT JobDescription, MAX(MissionCount) AS MostExperienced, MIN(MissionCount) AS LeastExperienced
FROM Personal
...
SELECT 
    CASE WHEN 'tf56sd#%OqH' LIKE '%[A-Z]%' THEN 'OH' END AS UppercaseLetters, 
    CASE WHEN 'tf56sd#%OqH' LIKE '%[a-z]%' THEN 'tfsdq' END AS LowercaseLetters,
    CASE WHEN 'tf56sd#%OqH' LIKE '%[0-9]%' THEN '56' END AS Numbers,
	CASE WHEN 'tf56sd#%OqH' NOT LIKE '%[A-Za-z0-9]%' THEN '#%' ELSE 0 END AS Others;
