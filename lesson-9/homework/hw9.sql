...
--Create working tables and load
...
--(1) Use Products, Suppliers table List all combinations of product names and supplier names.
...
SELECT c.CourseName, s.Name, s.Age, s.Major
FROM Courses c
...
SELECT p.ProductName, p.Category, o.Quantity, p.StockQuantity
FROM Orders o
INNER JOIN Products p ON o.ProductID = p.ProductID
...
SELECT e1.Name AS Employee1, e2.Name AS Employee2, d1.DepartmentName AS Dept1, d2.DepartmentName AS Dept2
FROM Employees e1
...
SELECT *
FROM Payments py
INNER JOIN Orders o ON py.OrderID = o.OrderID
INNER JOIN Products p ON o.ProductID = p.ProductID
WHERE py.Amount <> (o.Quantity * p.Price);
